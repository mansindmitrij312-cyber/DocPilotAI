...
Цель: соискание должности [Должность]
...
(Это демонстрационный пример — в развёрнутой версии с локальной AI-моделью документ формируется под точное описание пользователя.)""",
    "договор": """ДОГОВОР ОКАЗАНИЯ УСЛУГ №[Номер]

г. [Город]                                                   [Дата]

[Название организации], именуемое в дальнейшем «Исполнитель», с одной стороны, и [ФИО], именуемый в дальнейшем «Заказчик», с другой стороны, заключили настоящий договор о нижеследующем:

1. Предмет договора
Исполнитель обязуется оказать услуги по [Описание услуги], а Заказчик — принять и оплатить их.

2. Стоимость и порядок расчётов
Стоимость услуг составляет [Сумма] рублей.
...
Настоящим сообщается, что [Описание сути документа], в связи с чем [Дата].
...
def generate_document_demo(document_type):
    key = document_type.strip().lower()
    for k in DEMO_EXAMPLES:
        if k in key:
            return DEMO_EXAMPLES[k]
    return DEMO_EXAMPLES["default"]


def generate_document(document_type, description):

    if DEMO_MODE:
        return generate_document_demo(document_type)
